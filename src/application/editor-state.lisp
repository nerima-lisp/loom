...
reach any of them through *EDITOR-STATE* alone."
  ...
  ;; Running PTY-backed terminal sessions and their transcript buffers.
...
(defun add-before-save-hook (hook &optional (state *editor-state*))
  "Register HOOK before ordinary saves in STATE.
...
Registering the same function twice has no effect."
  ...
  (pushnew hook (editor-state-before-save-hooks state) :test #'eq)
  hook)

(defun remove-before-save-hook (hook &optional (state *editor-state*))
  "Remove HOOK from STATE and return HOOK.

Removing a function that is not registered is harmless."
  (when state
    (setf (editor-state-before-save-hooks state)
          (remove hook
                  (editor-state-before-save-hooks state)
                  :test #'eq)))
  hook)

(defun run-before-save-hooks (buffer &optional (state *editor-state*))
  "Run STATE's before-save hooks for BUFFER and return BUFFER.

The registration list is copied before dispatch, so hooks may update the
hook list without changing the current pass."
  (when state
    (dolist (hook (copy-list (reverse (editor-state-before-save-hooks state))))
      (funcall hook buffer)))
  buffer)

(defun add-after-save-hook (hook &optional (state *editor-state*))
  "Register HOOK for successful ordinary saves in STATE.

Hooks run in registration order and receive the saved buffer as their only
argument.  Registering the same function twice has no effect."
  (check-type hook function)
  (unless state
    (error "No editor state is active"))
  (pushnew hook (editor-state-after-save-hooks state) :test #'eq)
  hook)

(defun remove-after-save-hook (hook &optional (state *editor-state*))
  "Remove HOOK from STATE and return HOOK.

Removing a function that is not registered is harmless."
  (when state
    (setf (editor-state-after-save-hooks state)
          (remove hook
                  (editor-state-after-save-hooks state)
                  :test #'eq)))
  hook)

(defun run-after-save-hooks (buffer &optional (state *editor-state*))
  "Run STATE's after-save hooks for BUFFER and return BUFFER.

The registration list is copied before dispatch, so hooks may update the
hook list without changing the current pass."
  (when state
    (dolist (hook (copy-list (reverse (editor-state-after-save-hooks state))))
      (funcall hook buffer)))
  buffer)
