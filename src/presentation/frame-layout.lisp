...
Returns (VALUES CONTENT-HEIGHT MINIBUFFER-ROW FILE-TREE-WIDTH
WINDOW-AREA-WIDTH), leaving COMPOSE-FRAME itself to only sequence the draw
calls against them."
  (let* ((content-height (max 0 (1- height)))
    ...
    (values content-height minibuffer-row file-tree-width window-area-width)))
...
a left, up-to-24-column strip spanning every row above the minibuffer line;
...
very next frame -- draw every leaf window's buffer, each leaf's mode line, and
separators between adjacent leaves into that area; and finally draw the
minibuffer's current prompt/input or status line into the bottom row. Performs
