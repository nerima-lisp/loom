...
1. ~~**`next-line` の移動単位（FR-003）**~~ — **決定: 画面行単位**（Emacs の
   `line-move-visual` 既定 t 相当）。FR-003 の動機が Markdown / Org の文章執筆で
   ある以上、見えている通りに動くほうを採る。設定可能化は移動系コマンド全体に
   分岐を増やすため見送り。
2. ~~**表示方式のセッション永続化（FR-004 / NFR-004）**~~ — **決定: 永続化しない**。
   v5 エンベロープは据え置き。復帰時は major-mode の既定に戻り、
   `toggle-truncate-lines` での変更はその場限りとする。
